    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        if self.train:
            ...
            self.transform = transforms.Compose([
                transforms.Resize(224),  # 将 CIFAR 图像缩放到 224x224
                transforms.RandomHorizontalFlip(),  # 数据增强：随机水平翻转
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.48145466, 0.4578275, 0.40821073], std=[0.26862954, 0.26130258, 0.27577711]) #符合CLIP训练数据分布
            ])
        else:
            downloaded_list = self.test_list
            self.transform = transforms.Compose([
