...
def sub_set_save_path(args):
    if args.project == 'teen' or args.project == 'mine':
...
def set_save_path(args):
    ...
    if not args.not_data_init:
        mode = mode + '-' + 'data_init'
    args.save_path = os.path.join(args.dataset, args.project, '%s-start_%d' % (mode, args.start_session) + '_shot%d_' % (args.shot))
    ...
    args.save_path = os.path.join(args.dataroot, 'checkpoint', args.save_path)
